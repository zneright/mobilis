    ...
    useEffect(() => {
        const fetchOnChainDebt = async () => {
            ...
            try {
                ...
                // Build a read-only transaction calling your get_debt function
                ...
                // Simulate to get the return value without spending gas
                const simulation = await server.simulateTransaction(tx);

                if (rpc.Api.isSimulationSuccess(simulation)) {
                    // Decode the Rust i128 return value
                    const rawDebt = scValToNative(simulation.result.retval);
                    const parsedDebt = Number(rawDebt) / 10000000; // Convert 7-decimal stroops back to whole USDC
