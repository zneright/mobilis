...
export const CommuterRadar: React.FC<CommuterRadarProps> = ({ commuterData, currencyMode, setCurrencyMode, theme = 'dark' }) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto flex flex-col items-center space-y-5 font-sans">
            ...
            <div className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
                        <Radio className={`w-5 h-5 animate-pulse ${roleAccentText('commuter')}`} />
                        Transport Discovery
                    </h2>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 p-1 bg-white/90 dark:bg-[#07090E]/90 border border-gray-100 dark:border-white/10 rounded-full shadow-sm">
                        <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-bold transition-all ${
                                viewMode === 'map'
                                    ? `${roleCtaBg('commuter')} font-extrabold shadow-sm`
                    ...
                    >
                        <RefreshCw className={`w-3 h-3 ${roleAccentText('commuter')}`} /> {currencyMode}
                    </button>
                </div>
            </div>
            ...
            {viewMode === 'radar' && (
                <div className={`w-full rounded-3xl p-6 relative overflow-hidden flex flex-col items-center justify-center min-h-[320px] ${cardRoleStyle('commuter')}`}>
                    ...
                    <div className={`mt-4 flex items-center gap-2 text-xs font-mono font-bold px-4 py-1.5 rounded-full border ${rolePill('commuter')}`}>
                        ...
                        <span>
                            {nearbyDrivers.length} Driver{nearbyDrivers.length === 1 ? '' : 's'} ON TRANSIT within 200 meters
                        </span>
                    </div>
                </div>
            )}

            {/* NEARBY ON TRANSIT DRIVERS LIST (SONAR RADAR VIEW ONLY) */}
            {viewMode === 'radar' && (
                <div className="w-full pt-2">
                    <h3 className="text-base font-extrabold text-gray-900 dark:text-white mb-3 flex items-center justify-between">
                        ...
                        <span className={`text-xs font-mono font-bold ${roleAccentText('commuter')}`}>LIVE GPS BROADCAST</span>
                    </h3>
                    ...
                    {isLoadingDrivers ? (
                        ...
                        <div className="space-y-2.5">
                            {nearbyDrivers.map((drv) => (
                                <div
                                    key={drv.uid}
                                    className={`p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-all ${cardRoleStyle('commuter')}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-extrabold text-base shadow-sm border ${rolePill('commuter')}`}>
                                    ...
                                    <div className="w-full sm:w-auto flex items-center justify-between sm:justify-end gap-3">
                                        ...
                                        {commuterData.role !== 'driver' ? (
                                            <button
                                                onClick={() => setSelectedDriver(drv)}
                                                className={`px-4 py-2 font-extrabold text-xs rounded-full transition-all shadow-sm flex items-center gap-1.5 active:scale-95 ${roleCtaBg('commuter')}`}
                                            >
                                                <Zap className="w-3.5 h-3.5" /> Pay Fare
                                            </button>
                                        ) : (
                                            <span className={`px-3 py-1 rounded-full text-xs font-mono font-bold border ${rolePill('commuter')}`}>
                                                🛰️ Transit Active
                                            </span>
                                        )}
                                    </div>
                        ...
                        <div className="p-6 bg-white dark:bg-[#07090E] border border-gray-100 dark:border-white/10 rounded-2xl text-center space-y-1 font-mono">
                            ...
                            <h4 className="font-bold text-xs text-gray-900 dark:text-white">No Active Drivers Within 200 Meters</h4>
