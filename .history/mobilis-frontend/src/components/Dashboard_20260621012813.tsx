...
const Dashboard: React.FC = () => {
    ...
    const handleRequestAdvance = async () => {
        ...
        // Drivers are strictly locked to 15 USDC. Admins can inject custom values.
        ...
        try {
            ...
            // Check if it's the specific UnreachableCodeReached panic
            if (error.message?.includes("UnreachableCodeReached") || error.message?.includes("InvalidAction")) {
                alert("Transaction rejected by the Smart Contract. Ensure the TODA Treasury has funds and you do not have an active advance.");
            } else {
                alert("Network error. Failed to process advance.");
            }
        } finally {
                        ...
                        {isAdmin ? (
                            ...
                            <div className="w-full max-w-lg flex flex-col gap-4">
                                ...
                                <button
                                    ...
                                    className={`w-full py-5 rounded-2xl font-black text-lg tracking-wide shadow-xl transition-all ${debtState > 0 ? 'bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-gray-600 border border-gray-200 dark:border-white/5 cursor-not-allowed' : 'bg-gradient-to-r from-emerald-400 to-cyan-400 text-black hover:scale-[1.02] active:scale-[0.98]'}`}
                                >
