...
export const EarthCanvas = () => {
    ...
    useEffect(() => {
        ...
        const initThree = () => {
            ...
            // Pulled back from 2.8 -> 3.6 so the orbit ring (radius 1.3) and
            // atmosphere comfortably fit inside the view frustum instead of
            // being clipped at the sides.
            ...
            // ── ORBITAL RING ───────────────────────────────────────────
            // Radius brought in from 1.48 -> 1.3 (together with the camera
            // pull-back above) so the ring sits fully inside the visible
            // frustum instead of getting sliced off at the edges.
            ...
            // Remove shadow so the icon itself stays crisp
            tCtx.shadowBlur = 0;

            // Jeepney body (rounded rectangle)
            const bx = 18, by = 42, bw = 92, bh = 42, r = 10;
            tCtx.fillStyle = "#FBBF24"; // classic jeepney gold/yellow
            tCtx.beginPath();
            tCtx.moveTo(bx + r, by);
            ...
            // Windshield / window band
            tCtx.fillStyle = "#BAE6FD";
            tCtx.fillRect(bx + 6, by + 4, bw - 12, 14);

            // Colorful side stripe (signature jeepney decoration)
            ...
            const animate = () => {
                ...
                const orbitTilt = Math.PI * 0.35,
                    orbitR = 1.3; // matches the ring radius above
    ...
    return (
        ...
        >
            ...
            />
            {/* Removed rounded-full + overflow-hidden here: that hard circular
                mask was chopping the orbit ring and jeepney token off wherever
                they extended past the inscribed circle. The renderer itself
                already draws on a transparent background, so no CSS clip is
                needed. */}
